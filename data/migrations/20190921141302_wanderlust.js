...
exports.up = function (knex) {
    return knex.schema
        ...
        .createTable('experiences', tbl => {
            ...
            tbl.integer('organizer_id').references('id').inTable('organizers').notNullable().onDelete('CASCADE').onUpdate('CASCADE');
            tbl.integer('location_id').references('id').inTable('locations').notNullable().onDelete('CASCADE').onUpdate('CASCADE')
        })
        .createTable('guest_experiences', tbl => {
            tbl.integer('guest_id').references('id').inTable('guests').notNullable().onDelete('CASCADE').onUpdate('CASCADE');
            tbl.integer('experience_id').references('id').inTable('experiences').notNullable().onDelete('CASCADE').onUpdate('CASCADE');
            tbl.boolean('completed').defaultTo(false);
        ...
        .createTable('experience_types', tbl => {
            tbl.integer('experience_id').references('id').inTable('experiences').notNullable().onDelete('CASCADE').onUpdate('CASCADE');
            tbl.integer('type_id').references('id').inTable('types').notNullable().onDelete('CASCADE').onUpdate('CASCADE');
